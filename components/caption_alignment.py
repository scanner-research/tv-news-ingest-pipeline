...
from util.utils import get_base_name, save_json, format_hmmss
...
#                 align_word_list.append((word['word'], word['start'] + audio_start, shift))
    ...
    def estimate_shift_all(self):
        ...
        """
        print('Estimating shift window')
        sys.stdout.flush()
        with Pool(self.num_thread) as workers:
    ...
    def run(self):
        ...
        print('Aligning captions without estimating shift')
        sys.stdout.flush()
        with Pool(self.num_thread) as workers:
            results = [
                workers.apply_async(
        ...
        if self.estimate and missing_rate > self.missing_thresh:
            ...
            print('Aligning captions with estimating shift')
            sys.stdout.flush()
            with Pool(self.num_thread) as workers:
                results = [
                    workers.apply_async(
...
def main(video_in_path, transcript_in_path, out_path, force=False):
    ...
    for i in range(len(video_names)):
        print("Aligning captions for video '{}' ({:0.1f} % done, {} elapsed)".format(
            video_names[i], i / len(video_names) * 100,
    ...
    print('Done aligning captions ({} elapsed)'.format(
        format_hmmss(time.time() - start_time)))
    sys.stdout.flush()

    # Clean up temp files generated by gentle
    import glob
    tmpfiles = glob.glob('/tmp/*.aac')
    tmpfiles += glob.glob('/tmp/*_HCLG.fst')
    for f in tmpfiles:
        os.remove(f)
